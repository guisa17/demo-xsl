...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:param name="filtro_nombre" />

  <xsl:template match="/">
    <html>
      <head>
        <title>Resultados para "<xsl:value-of select="$filtro_nombre" />"</title>
        <style>
          * {
            margin: 0;
            padding: 0;
          ...
          h1 { 
            background: linear-gradient(135deg, #2E86C1 0%, #2E86C1 100%);
            color: white; 
            padding: 20px;
      ...
      <body>
        <div class="container">
          <h1>Resultados para "<xsl:value-of select="$filtro_nombre" />"</h1>
          <div class="table-container">
            <table>
              ...
              <tbody>
                <xsl:for-each select="/response/row/row[contains(translate(station_name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), $filtro_nombre)]">
                  <tr>
